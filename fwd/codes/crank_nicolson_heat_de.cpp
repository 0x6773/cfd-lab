...
int main()
{
    ...
    for (int j = 1; j <= 200; j++) {
        d[1] = (1.0 - 2 * r) * u[1][j - 1] + r * u[0][j - 1] + r * u[2][j - 1]
               + r * u[0][j];
        for (int i = 2; i < x; i++) {
            d[i] = (1.0 - 2 * r) * u[i][j - 1] + r * u[i - 1][j - 1]
                   + r * u[i + 1][j - 1];
        }
        d[x] = (1.0 - 2 * r) * u[x][j - 1] + r * u[x - 1][j - 1]
               + r * u[x + 1][j - 1] + r * u[x - 1][j];

        d_dash[1] = d[1] / b[1];
        for (int i = 2; i <= x; i++) {
            d_dash[i]
                = (d[i] - a[i] * d_dash[i - 1]) / (b[i] - a[i] * c_dash[i - 1]);
        }
        ...
        if (j == 100) {
            for (int i = 0; i <= x; i++) {
                cout << "Approximate values :" << u[i][100]
                     << " Actiual Value: "
                     << 100.0 * exp(-1.0 * pow(M_PI, 2) * 0.5)
                            * sin(M_PI * (i * x_step))
                     << endl;
            }
        }
